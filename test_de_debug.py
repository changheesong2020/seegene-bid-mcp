...
async def debug_de_save():
    """DE 크롤러 저장 로직 디버깅"""
    print("="*60)
    print("DE Crawler Save Logic Debug")
    print("="*60)
    ...
    try:
        ...
        # DE 크롤러 인스턴스 생성
        crawler = GermanyVergabestellenCrawler()

        # 작은 범위로 테스트 (단일 키워드)
        print("\n1. Testing DE Crawler with Single Keyword")
        test_keywords = ["medizinisch"]  # 독일어 의료 키워드
        ...
        try:
            # 간단한 테스트 데이터 생성
            test_data = [{
                "title": "Test Medizinische Ausrüstung Procurement",
                "organization": "Test German Hospital",
                "bid_number": "DE-TEST-001",
                "announcement_date": "2025-01-01",
                "deadline_date": "2025-02-01",
                "estimated_price": "80000",
                "currency": "EUR",
                "source_url": "https://test.de/test",
                "source_site": "DE_VERGABESTELLEN",
                "country": "DE",
                "relevance_score": 8.0,
                "urgency_level": "medium",
                "status": "active",
                "keywords": ["medizinisch"],
        ...
        try:
            ...
            if 'site_breakdown' in stats:
                de_count = stats['site_breakdown'].get('DE_VERGABESTELLEN', 0)
                print(f"   DE_VERGABESTELLEN records: {de_count}")
        except Exception as e:
    ...
    print(f"\n" + "="*60)
    print("DE Debug Complete")

if __name__ == "__main__":
    asyncio.run(debug_de_save())