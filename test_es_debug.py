...
async def debug_es_save():
    """ES 크롤러 저장 로직 디버깅"""
    print("="*60)
    print("ES Crawler Save Logic Debug")
    print("="*60)
    ...
    try:
        ...
        # ES 크롤러 인스턴스 생성
        crawler = SpainPCSPCrawler()

        # 작은 범위로 테스트 (단일 키워드)
        print("\n1. Testing ES Crawler with Single Keyword")
        test_keywords = ["medico"]  # 스페인어 의료 키워드
        ...
        try:
            # 간단한 테스트 데이터 생성
            test_data = [{
                "title": "Test Equipamiento Médico Procurement",
                "organization": "Test Spanish Hospital",
                "bid_number": "ES-TEST-001",
                "announcement_date": "2025-01-01",
                "deadline_date": "2025-02-01",
                "estimated_price": "50000",
                "currency": "EUR",
                "source_url": "https://test.es/test",
                "source_site": "ES_PCSP",
                "country": "ES",
                "relevance_score": 8.0,
        ...
        try:
            ...
            if 'site_breakdown' in stats:
                es_count = stats['site_breakdown'].get('ES_PCSP', 0)
                print(f"   ES_PCSP records: {es_count}")
        except Exception as e:
    ...
    print(f"\n" + "="*60)
    print("ES Debug Complete")

if __name__ == "__main__":
    asyncio.run(debug_es_save())