...
async def debug_it_save():
    """IT 크롤러 저장 로직 디버깅"""
    print("="*60)
    print("IT Crawler Save Logic Debug")
    print("="*60)
    ...
    try:
        ...
        # IT 크롤러 인스턴스 생성
        crawler = ItalyMEPACrawler()

        # 작은 범위로 테스트 (단일 키워드)
        print("\n1. Testing IT Crawler with Single Keyword")
        test_keywords = ["medico"]  # 이탈리아어 의료 키워드
        ...
        try:
            # 간단한 테스트 데이터 생성
            test_data = [{
                "title": "Test Equipaggiamento Medico Procurement",
                "organization": "Test Italian Hospital",
                "bid_number": "IT-TEST-001",
                "announcement_date": "2025-01-01",
                "deadline_date": "2025-02-01",
                "estimated_price": "75000",
                "currency": "EUR",
                "source_url": "https://test.it/test",
                "source_site": "IT_MEPA",
                "country": "IT",
                "relevance_score": 8.0,
        ...
        try:
            ...
            if 'site_breakdown' in stats:
                it_count = stats['site_breakdown'].get('IT_MEPA', 0)
                print(f"   IT_MEPA records: {it_count}")
        except Exception as e:
    ...
    print(f"\n" + "="*60)
    print("IT Debug Complete")

if __name__ == "__main__":
    asyncio.run(debug_it_save())